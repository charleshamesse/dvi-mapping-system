...
ros::Publisher pub_path, pub_relo_path, pub_pose_covariance_stamped;
...
void registerPub(ros::NodeHandle &n)
{
    ...
    pub_pose_covariance_stamped =  n.advertise<geometry_msgs::PoseWithCovarianceStamped>("pose_cov_stamped", 1000);
...
void pubPoseWithCovarianceStamped(const Estimator &estimator, const std_msgs::Header &header, int n_features, int n_features_max)
{
    ...
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        Quaterniond R = Quaterniond(estimator.Rs[i]); // * estimator.ric[0]);
        /*
        Matrix3d Rm = R.toRotationMatrix();
        std::cout << "rotation vins mono" << std::endl;
        std::cout << Rm << std::endl;
        Matrix3d R_vins_to_devil;
        R_vins_to_devil << 
            1,0,0, 
            0,1,0, 
            0,0,1;
        Rm = R_vins_to_devil * Rm; 
        P = R_vins_to_devil * P;
        R = Quaterniond(Rm);*/
        ...
        cov.setIdentity();
        // cov-1:
        // double cov_multiplier = 1;
        // cov-ada-100:
        double cov_multiplier = 1 + pow(((n_features_max - n_features)/1.), 1); // or 1000?
        // double cov_multiplier = 1 + pow(10, (n_features_max - n_features)/n_features_max); // or 1000?
        // n_features:
        ROS_INFO("[vins-mono] cov multiplier: %f", cov_multiplier);
        // no ada: double cov_multiplier = 1;
        ROS_INFO("[vins-mono] publishing pose, using n_features: %d, covariance multiplier %.2f", n_features, cov_multiplier);
        ROS_INFO("[vins-mono] publishing pose, using timestamp: %f", pose_with_cov_stamped.header.stamp.toSec());
        cov = 0.0000005 * cov * cov_multiplier;
        for(int i = 0; i < 36; i++) {
