...
void transformLidar(const StatesGroup &state,
                    const shared_ptr<ImuProcessing> &p_imu,
                    const PointCloudXYZI::Ptr &input_cloud, // is actually XYZINormal
                    pcl::PointCloud<pcl::PointXYZ>::Ptr &trans_cloud) {
  trans_cloud->clear();
  for (size_t i = 0; i < input_cloud->size(); i++) {
    pcl::PointXYZINormal p_c = input_cloud->points[i];
    Eigen::Vector3d p(p_c.x, p_c.y, p_c.z);
    p = p_imu->Lid_rot_to_IMU * p + p_imu->Lid_offset_to_IMU; // CH here
...
void transformLidar(const StatesGroup &state,
                    const shared_ptr<ImuProcessing> &p_imu,
                    const PointCloudType::Ptr &input_cloud, // is actually XYZINormal
                    pcl::PointCloud<pcl::PointXYZ>::Ptr &trans_cloud) {
  trans_cloud->clear();
  for (size_t i = 0; i < input_cloud->size(); i++) {
    PointType2 p_c = input_cloud->points[i];
    Eigen::Vector3d p(p_c.x, p_c.y, p_c.z);
    p = p_imu->Lid_rot_to_IMU * p + p_imu->Lid_offset_to_IMU; // CH here
    p = state.rot_end * p + state.pos_end;
    pcl::PointXYZ pi;
...
void transformLidar(const StatesGroup &state,
                    const shared_ptr<ImuProcessing> &p_imu,
                    const PointCloudType::Ptr &input_cloud, // is actually XYZINormal
                    PointCloudType::Ptr &trans_cloud) {
  trans_cloud->clear();
  for (size_t i = 0; i < input_cloud->size(); i++) {
    PointType2 p_c = input_cloud->points[i];
    Eigen::Vector3d p(p_c.x, p_c.y, p_c.z);
